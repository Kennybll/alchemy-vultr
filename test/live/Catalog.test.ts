...
test("listRegions returns regions with id/city/country", async () => {
  const regions = await Effect.runPromise(
    withClient(Vultr.Catalog.listRegions()),
  );
  ...
  expect(Array.isArray((ewr as { connectivity?: unknown }).connectivity)).toBe(
    true,
  );
});
...
test("listPlans filters by type=vhf", async () => {
  const plans = await Effect.runPromise(
    withClient(Vultr.Catalog.listPlans({ type: "vhf" })),
  );
...
test("listBareMetalPlans uses plans_metal collection key", async () => {
  const plans = await Effect.runPromise(
    withClient(Vultr.Catalog.listBareMetalPlans()),
  );
  expect(plans.length).toBeGreaterThan(0);
...
test("listOperatingSystems and listApplications", async () => {
  const os = await Effect.runPromise(
    withClient(Vultr.Catalog.listOperatingSystems()),
  );
  const apps = await Effect.runPromise(
    withClient(Vultr.Catalog.listApplications()),
  );
...
test("listObjectStorageClusters is public; tiers require auth", async () => {
  const clusters = await Effect.runPromise(
    withClient(Vultr.Catalog.listObjectStorageClusters()),
  );
...
test("listKubernetesVersions returns version strings with +build suffix", async () => {
  const response = await Effect.runPromise(
    withClient(Vultr.Catalog.listKubernetesVersions()),
  );
...
test.skipIf(!hasApiKey)(
  "getAccount works when IP allowlist permits this egress",
  async () => {
    const access = await probeAuthenticatedAccess();
    if (!access.ok) {
    ...
    const account = await Effect.runPromise(
      withClient(Vultr.Catalog.getAccount()),
    );
    expect(account).toBeDefined();
    expect(
      typeof (account as { account?: { email?: string } }).account?.email ===
        "string" || typeof account === "object",
    ).toBe(true);
  },
);
