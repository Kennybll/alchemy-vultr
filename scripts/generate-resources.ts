...
${toPropsInterface(def)}

export type ${def.name}Attributes = {
${attrLines.join("\n")}
};
 ...
 * @resource
 */
export const ${def.name} = defined.Resource;
export const ${def.name}Provider = defined.Provider;
`;
};


const outDir = join(import.meta.dir, "../src/resources");
...
for (const def of resources) {
  ...
  const path = join(outDir, `${def.file}.ts`);
  writeFileSync(path, content);
  exportNames.push(def.name);
...
writeFileSync(
  ...
  `${exportNames.map((n) => `export * from "./${n}.ts";`).join("\n")}\n`,
);

// Write a machine-readable list for Providers.ts generation
writeFileSync(
  join(outDir, "_catalog.json"),
  JSON.stringify(
    resources.map((r) => ({ name: r.name, file: r.file, type: r.type })),
    null,
    2,
  ),
);
