...
| Instance `main_ip` | Create/get return the instance object with `main_ip` | **`main_ip` is `"0.0.0.0"` until Vultr assigns IPv4** — the create response and the first GETs after it can all carry the placeholder while `status` is `pending`. Returning it publishes an unroutable address (a `0.0.0.0` A record). The provider polls until `main_ip` is a routable public IPv4, or fails with `VultrNotReady`. |
| Instance create + retries | `POST /instances` is a normal create | Non-idempotent: a lost response or ambiguous 5xx that gets retried can provision **another VM**. The provider posts exactly once per reconcile (`client.postOnce`), then polls only its ownership tag (`alchemy-vultr-recover-…`) for a bounded window. Empty lists never authorize another create POST; expiry fails closed with `VultrCreateUncertain`. |
