...
| Instance create + retries | `POST /instances` is a normal create | Non-idempotent: a lost response or ambiguous 5xx that gets retried provisions **another VM**. The provider posts once (`client.postOnce`) and looks for its ownership tag (`alchemy-vultr-recover-…`) after every ambiguous failure before posting again. |
